...
class ItemBias:
    ...
    def fit(self, reviews):
        ...
        for u, i, r in reviews:
            self.biases[i] = self.biases.get(i, 0) + r
    ...
    def select(self, reviews):
        ...
        for t in range(10):
            self.threshold = t
            predictions = [self.predict(i) for u, i, r in reviews]
    ...
    def predict(self, item):
        if self.counts.get(item, 0) > self.threshold:
            return self.biases[item] / self.counts[item]
        else:
            return self.overall
