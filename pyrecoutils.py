...
def rmse(reviews, predictions):
    return np.sqrt(np.average([(r - p) ** 2 for (u, i, r), p in zip(reviews, predictions)]))
...
def load_texts(data_filename):
    logging.info(u"Loading ratings from %s", data_filename)
    texts = []
    with gzip.open(data_filename, u"rb") as data_file:
        for line in data_file:
            tokens = line.decode(u"UTF-8", u"ignore").strip().split()
            nb_words = int(tokens[4])
            words = tokens[5:]
            assert nb_words == len(words)
            texts.append(words)
    logging.info(u"Loaded %d texts from %s", len(texts), data_filename)
    return texts
...
if __name__ == u"__main__":
    ...
    loaded_reviews = load_ratings(sys.argv[1])
    logging.info(u"Number of users: %d", len(set([u for u, i, r in loaded_reviews])))
    logging.info(u"Number of items: %d", len(set([i for u, i, r in loaded_reviews])))
    logging.info(u"Average rating: %f", np.mean([r for u, i, r in loaded_reviews]))